...
**Caffeine caches need reflection hints.** `Caffeine.build()` does not instantiate `BoundedLocalCache` directly: `LocalCacheFactory` derives a class *name* from the builder options chosen at runtime (`SSMSA` for `maximumSize + expireAfterAccess`, as used by `UserChatClientResolver`) and resolves it with `MethodHandles.Lookup.findClass`, then `findStaticVarHandle(clazz, "FACTORY")` with `findConstructor` as fallback. All three take a non-constant class, so native-image cannot intrinsify them and drops the ~376 generated classes as unreachable — the first `build()` then dies at runtime with `IllegalStateException: SSMSA` / `ClassNotFoundException: com.github.benmanes.caffeine.cache.SSMSA` (fine on the JVM, so only native deployments hit it). `CaffeineRuntimeHints` scans the caffeine jar and registers every generated class (names built only from the option letters `S W I L M A R`) with declared fields + constructors.
