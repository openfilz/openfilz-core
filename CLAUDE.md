...
See `docs/pdf-tools.md`. One PDFBox primitive — `service/pdf/PdfCompositionEngine` (a *page composition*:
the output is exactly the listed `(source, page, rotation)` refs; split = N compositions; temp-file stream
cache, sources kept open until save) — orchestrated by `PdfToolsServiceImpl`: sources read via
`findDocumentToDownloadById` (`AccessType.RO`), limits from `PdfToolsProperties` (`openfilz.pdf-tools.*`,
semaphore-bounded on `boundedElastic`), write-back **only** through `uploadDocument` / `replaceDocumentContent`
(`PathFilePart`) so audit/versioning/checksum/quota/thumbnails/indexing apply, plus an `AuditAction.PDF_TRANSFORM`
entry (`PdfTransformAudit`: operation, source ids) on every output; top-level bookmarks are re-pointed at the
surviving pages. Guards: `PdfToolsException` (422 not-a-PDF / encrypted / too many pages or outputs; 409
signed-without-acknowledgement / active e-Sign envelope / WORM in-place; 503 busy). REST `PdfToolsController` at
`/api/v1/pdf` (`info`, `merge`, `split`, `organize`, `rotate`) — runtime toggle `openfilz.pdf-tools.active`
(**default on**, 404 when off, `Settings.pdfToolsActive`; no `@Valid` on the bodies so the 404 wins over
validation — the service validates and answers 400); `AbstractSecurityService` allow-lists `/pdf/**` (GET →
READER/CONTRIBUTOR, POST → CONTRIBUTOR; WORM keeps POST but the service refuses `NEW_VERSION`). Page selection
grammar in `PageRangeParser` (`1-3,7,10-`, odd/even/all). AI/MCP: `PdfAiTools` (7 `@Tool`s, self-enforcing
`AiToolRolePolicy` + `AiAccessPolicy`) registered by `PdfAiToolsContributor` (+ `PdfAiToolsRuntimeHints` for
native) — `McpToolContributor.exposeInChat()` is the opt-in that also binds a contributor's tools into the chat
assistant (`ChatClientAssembler.assemble(model, docTools, extraTools)`); only contributors whose tools gate roles
themselves may opt in. Tests: `PdfCompositionEngineTest`, `PageRangeParserTest`, `PdfToolsServiceImplTest`
(guards/naming/provenance with mocks), `e2e/pdf/PdfToolsIT` (Keycloak roles, all flows, PDFs generated by
`PdfTestFiles`), `e2e/pdf/PdfAiToolsIT` (the same engine through the 7 `@Tool` methods over MCP: name/folder
resolution, the split-mode and page-selection vocabulary, in-place vs. new-document routing, READER refused),
...
- **Provider `enabled` flags are gone.** `spring.ai.<provider>.chat.enabled` no longer exists; each provider's auto-configuration is gated on `spring.ai.model.chat` / `spring.ai.model.embedding` (value = provider name, or `none`), and those conditions are `matchIfMissing = true` — so with both the Ollama and OpenAI starters on the classpath and no selector set, **both** providers' models get created. `AiModelProviderEnvironmentPostProcessor` derives the selectors so that `openfilz.ai.active` is the single switch: off ⇒ everything `none` (nothing is built, matching the beans, which are all conditional on that flag); on ⇒ chat and embedding resolve independently from the `openfilz.ai.<provider>.<kind>.enabled` booleans (`OLLAMA_CHAT_ENABLED` / `OPENAI_CHAT_ENABLED` …), Ollama winning ties and serving as the fallback when a kind has no switch, since its defaults target a stock local install. An explicit `spring.ai.model.*` always wins. **The recommended setup is one pair** — `OPENFILZ_AI_MODEL=provider:model` + `OPENFILZ_AI_API_KEY` (BYOK default; `openfilz-cloud:default` = managed gateway for insights + smart filing only, chat selector `none`): chat precedence is selector > `<PROVIDER>_CHAT_ENABLED` > the pair > first chain entry > Ollama, and the pair is translated into `openfilz-internal.ai.{chat-model,api-key}.<selector>` / `openfilz-internal.ai.{insights-model,cloud-api-key}` nested placeholder defaults, so the vendor variables still win. Deployment targets pass vendor keys/models **only when set** (bare Compose keys, Helm `with`) — an empty env var would shadow the derived value. The pair never implies `openfilz.ai.active`; a bad value is warned about by `AiFallbackValidator`. The post-processor must stay ordered after `ConfigDataEnvironmentPostProcessor` or the switches read as absent. Tests that mock the models pin `spring.ai.model.*=none`, or the real models collide with the mocks.
- **Embedding providers go through `EmbeddingModels`, never through an `EmbeddingModel` injection.** Three providers: in-process
  (`TRANSFORMERS_EMBEDDING_ENABLED`, nomic-embed-text-v1.5 through ONNX Runtime inside the API — `spring-ai-transformers` only,
  never its auto-configuration), Ollama, OpenAI-compatible (TEI through `OPENAI_BASE_URL`). `EmbeddingModels.effective()` builds the
  in-process model at runtime from the flag — no bean, no condition — so the EE native image (selector fixed at build time = Ollama)
...
- **`openfilz.ai.chat.active` is the chat-only kill switch** (default true, read per request). Off = `AiChatController` + `AiSettingsController` answer 404 and `Settings.aiChatActive` is false, so the web apps hide the chat FAB, the panel and "Organise with AI" (`SettingsService.isAiChatActive`, `AiOrganizeAccessService`); embeddings, insights, smart filing, `by-kind` reorganisation and MCP are untouched. It exists so a `prototype`/`learned` deployment can also set `spring.ai.model.chat=none` and run with **no LLM**: `UserChatClientResolver` takes `ObjectProvider<ChatModel>` (never a required injection) so `AiDocumentInsightService` / `DefaultAutoFileService` still build, and only an actual model request fails, naming what needs one. `AiChatDisabledIT` pins it (chat+BYOK 404, `aiActive` true / `aiChatActive` false, by-kind still answering with no `ChatModel` bean). `Settings.aiChatUnavailableReason` says why the chat is off while AI is on: `DISABLED` (this switch) or `NO_MODEL` (`UserChatClientResolver.hasDefaultChatModel()` false — e.g. `openfilz-cloud` alone or `spring.ai.model.chat=none`; a recognised provider bean compiled into a native image is ignored under a runtime `none`, mocks are trusted); `aiChatActive`/`aiUserSettingsEnabled` are false in both cases — except that per-user BYOK (`openfilz.ai.user-settings.enabled`) never reports `NO_MODEL`, since each user can bring their own key. See `docs/ai-overview.md` §6.3.
...
- **The suite trims the chat tool surface.** `openfilz.ai.chat.excluded-contributors=OrganizeAiToolsContributor,SignatureAiToolsContributor` (`AiRealLlmE2EIT`) keeps the 1.5B model on the document + PDF tools it was measured with: with the 34-tool surface it answered *"I can't assist with that"* or emitted the tool call as text (~2 min per request on CI). The knob is a real deployment setting (small local models), see `docs/ai.md`; e-Sign tools are additionally bound only while `openfilz.signature.active` is on. Since the 19th document tool (`getDocumentActivity`) the suite also sets `openfilz.ai.chat.excluded-tools=getDocumentActivity` to stay on the measured 25-tool surface — **every tool added to `DocumentAiTools` must be excluded there too**, or CI regresses (run 33922064914).
...
- **Tool surface (core):** 19 document tools in `DocumentAiTools` (incl. `createBlankDocument` and `getDocumentActivity` — the audit trail, AUDITOR-gated), plus two more core contributors — `OrganizeAiToolsContributor` (4 reorganisation tools over `ReorganizationPlanService`: inventory → validated, persisted proposal → user-confirmed apply; the chat appends a `[[reorg-plan:id]]` marker rendered as a proposal card, `/api/v1/ai/reorganization/{id}`; contributed tool objects report side effects via `AiToolTurnEffects`) and `SignatureAiToolsContributor` (4 e-Sign tools: `sendForSignature` via template role binding or default last-page placement, list/status) — whoami (identity + effective
  ...
  CLEANER-gated `deleteDocument` — actually dispatches for the layer-2 trace. The contributed
  surfaces have their own end-to-end suites beside the happy paths: `ReorganizationEdgeCasesIT`
  (inventory limits, malformed plans, argument guards, re-validation on apply, ownership) and
  `SignatureAiToolsLookupIT` (envelope resolution by title, listing filters, empty shelves).
...
- **Tests:** `McpRuntimeHintsTest` (hints), `McpProtocolIT` (read-write, full protocol),
  ...
  `DocumentAiToolsMcpIT` (the document tools against real storage and DB: resolution by partial
  name, the folder-scoped search, metadata/version round-trips, the download link),
  `DocumentAiToolsVisionIT` (`describeImage` per task and the PDF page-rendering path, against
  `AiTestConfig`'s mocked model), sharing `AbstractMcpIT`'s JSON-RPC client.
