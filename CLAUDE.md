...
The AI feature (`openfilz.ai.active=true`) runs on Spring AI 2.0. Four things bite when upgrading from 1.x:
- **Provider `enabled` flags are gone.** `spring.ai.<provider>.chat.enabled` no longer exists; each provider's auto-configuration is gated on `spring.ai.model.chat` / `spring.ai.model.embedding` (value = provider name, or `none`), and those conditions are `matchIfMissing = true` — so with both the Ollama and OpenAI starters on the classpath and no selector set, **both** providers' models get created. OpenFilz keeps the documented `OLLAMA_CHAT_ENABLED` / `OPENAI_CHAT_ENABLED` booleans (now under `openfilz.ai.<provider>.<kind>.enabled`) and `AiModelProviderEnvironmentPostProcessor` derives the selectors from them; it must stay ordered after `ConfigDataEnvironmentPostProcessor` or the switches read as absent. Tests that mock the models must pin `spring.ai.model.*=none`, or the real models collide with the mocks.
- **Tool execution moved to a `ToolCallingAdvisor`** on the ChatClient. Only the *auto-configured* `ChatClient.Builder` registers it — a client built from `ChatClient.builder(chatModel)` emits tool-call requests nobody executes. `AiConfig` registers the advisor explicitly.
