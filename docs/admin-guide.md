...
| `OPENFILZ_AI_INSIGHTS_LOCAL_CONCURRENCY` | `8` | Parallel enrichments when `OPENFILZ_AI_INSIGHTS_CLASSIFIER` is `prototype` or `learned`: those modes call no model, so nothing but CPU limits them and an upload batch is classified far faster than the model concurrency above would allow |
...
| `OPENFILZ_AI_AUTO_FILE_TEXT_HANDOFF_TTL` | `10m` | How long an entry nobody took survives. The normal life of an entry is the seconds between the upload's parse and the filing that consumes it |
| `OPENFILZ_AI_AUTO_FILE_SESSION_GRACE` | `30m` | How long after the uploader's access token expires a filing queued while it was still valid may still run. A long upload batch outlives a short-lived access token (Keycloak's default is 5 min): without the grace the tail of the batch is skipped with *"the session expired before the document could be filed"*. The token is never sent anywhere — it only carries the identity the move is audited and authorised under. `0` = no grace |
