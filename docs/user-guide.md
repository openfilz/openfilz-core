...
> This feature is **optional** and must be enabled by your administrator. If the AI chat is not visible in your interface, it means it has not been activated for your instance.

When enabled, OpenFilz includes an AI assistant that understands your documents. You can ask it questions in natural language, and it will search through your document library to provide relevant answers.

### What the AI Can Do

- **Answer questions** about your documents using their actual content
- **Search and find** documents by describing what you're looking for
- **Summarize** document content
- **Organize files** — create folders, move and rename documents on your behalf
- **Provide context** — explain what a document contains without you having to open it
...
4. Continue the conversation with follow-up questions — the AI remembers the context

### Example Prompts

- "What documents do I have about the Q3 report?"
- "Summarize the content of the latest invoice"
- "Create a folder called Archive and move all 2024 files there"
- "How many files are in my project folder?"

### Conversations

- Each chat session creates a **conversation** that is saved automatically
- You can return to previous conversations to review past answers
- Delete old conversations when they are no longer needed

### Important Notes

- The AI assistant can only access documents stored in OpenFilz
- Answers are generated by a language model and may occasionally be inaccurate — always verify important information
- The AI respects the same access permissions as your user account

---

## Electronic Signature

> This feature is **optional** and must be enabled by your administrator. If you do not see a
> **Request signature** action on your PDFs, it has not been activated on your instance.

OpenFilz can send a PDF you already store here to one or more people for electronic signature,
then file the signed result straight back into your folders.

### Sending a document for signature

1. Select a PDF in the file explorer (or open its menu) and choose **Request signature**
2. Add the recipients: **signers**, who must act, and **CC** recipients, who simply receive the
   completed document
3. Choose the order — everyone at once, or one after another, each step unlocking the next
4. Drag the fields onto the pages and assign each one to a recipient
5. Optionally require an **email one-time code** for a given recipient, set an expiry, and send

You can also save a **template**: the roles and the field placements are stored once, so sending
the same contract to new people later takes a single step.

### Field types

Signature, initials, date, free text, number, email, phone, checkbox, radio button, dropdown,
image and stamp. The date field is filled automatically at signing time.

### What the signer sees

Recipients receive an email with a personal link — **no OpenFilz account is required**. The
signing page opens in their own language (8 available, Arabic included, right-to-left), shows
only the fields assigned to them, and lets them draw, type or upload their signature and
initials. They can also decline, giving a reason.

### Following an envelope

- Track who has signed and who is still pending
- **Resend** a link to someone who lost it — the previous link stops working immediately
- **Cancel** an envelope that is no longer relevant
- An envelope past its expiry date closes itself

### When everyone has signed

- A **Certificate of Completion** is appended to the PDF: each signer, timestamps, IP address,
  the authentication method used, and the full event trail
- The document is **sealed cryptographically**, so any later alteration is detectable
- The signed copy is saved next to the original as `<name> (signed).pdf`, in read-only mode, and
  emailed to everyone involved

### Important notes

- Only PDF documents can be sent for signature
- The signed copy is a new document — the original is left untouched
- Every step is recorded in the [Audit Trail](#audit-trail)

> **Administrators:** the configuration reference (activation, email, one-time codes, seal
> certificates) is in the [e-Sign Guide](esign.md).
